                ...
                <Form {...form}>
                    <form className="space-y-8" onSubmit={form.handleSubmit(handleSaveProfile)}>
                        <Tabs defaultValue="perfil" className="w-full" value={activeTab} onValueChange={setActiveTab}>
                                ...
                                <Card className="bg-card/50">
                                    <CardHeader>
                                        <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                                            ...
                                            <div className="space-y-1">
                                                ...
                                                <CardDescription className="text-xs">Cálculo de idade metabólica e carga de impacto.</CardDescription>
                                            </div>
                                        </div>
                                    </CardHeader>
                                    <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-border/50">
                                        ...
                                        <FormField control={form.control} name="location" render={({field}) => (
                                            <FormItem>
                                                <FormLabel className="text-xs font-bold uppercase">Localização</FormLabel>
                                    ...
                                    <CardContent className="space-y-6 pt-6 border-t border-border/50">
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            ...
                                            <FormField control={form.control} name="thresholdPace" render={({field}) => (
                                                <FormItem>
                                                    <FormLabel className="text-xs font-bold uppercase">Pace Limiar (MM:SS)</FormLabel>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t pt-6">
                                            ...
                                            <FormField control={form.control} name="planGenerationType" render={({field}) => (
                                                <FormItem>
                                                    <FormLabel className="text-xs font-bold uppercase">Estratégia de Geração</FormLabel>
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <FormControl><SelectTrigger className="bg-secondary/10"><SelectValue/></SelectTrigger></FormControl>
                                                        <SelectContent>
                                                            <SelectItem value="blocks">Blocos (4 semanas)</SelectItem>
                                                            <SelectItem value="full">Ciclo (Até a prova)</SelectItem>
                                                        </SelectContent>
                                                    </Select>
                                                </FormItem>
                                            )} />
                            ...
                            <TabsContent value="alimentacao" className="mt-6 space-y-6 animate-in fade-in">
                                <Card className="bg-card/50">
                                    ...
                                    <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-border/50">
                                        ...
                                        <FormField control={form.control} name="mealCount" render={({field}) => (
                                            <FormItem>
                                                <FormLabel className="text-xs font-bold uppercase">Refeições Diárias</FormLabel>
                                                <FormControl><Input type="number" {...field} value={field.value ?? ''} className="bg-secondary/10" /></FormControl>
                                            </FormItem>
                                        )} />
                                        ...
                                        <FormField control={form.control} name="excludedFoods" render={({field}) => (
                                            <FormItem className="sm:col-span-2">
                                                <FormLabel className="text-xs font-bold uppercase">Restrições / Excluídos</FormLabel>
                                                <FormControl><Textarea placeholder="Alimentos a serem ignorados..." {...field} value={field.value ?? ''} className="bg-secondary/10" /></FormControl>
                                            </FormItem>
                                        )} />
                                    </CardContent>
                                </Card>
                            ...
                            <TabsContent value="musculacao" className="mt-6 space-y-6 animate-in fade-in">
                                <Card className="bg-card/50">
                                    <CardHeader><CardTitle className="font-headline text-xl md:text-2xl uppercase italic">A Blindagem de Elite</CardTitle></CardHeader>
                                    <CardContent className="space-y-6 pt-6 border-t border-border/50">
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            <FormField control={form.control} name="prBench" render={({field}) => (
                                                <FormItem><FormLabel className="text-[10px] font-bold uppercase">Supino PR</FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ''} /></FormControl></FormItem>
                                            )} />
                                            <FormField control={form.control} name="prSquat" render={({field}) => (
                                                <FormItem><FormLabel className="text-[10px] font-bold uppercase">Agach. PR</FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ''} /></FormControl></FormItem>
                                            )} />
                                            <FormField control={form.control} name="prDeadlift" render={({field}) => (
                                                <FormItem><FormLabel className="text-[10px] font-bold uppercase">Terra PR</FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ''} /></FormControl></FormItem>
                                            )} />
                                        </div>
                                            ...
                                            <FormField control={form.control} name="legDay" render={({field}) => (
                                                <FormItem>
                                                    <FormLabel className="flex items-center gap-2 text-xs font-bold uppercase">
                                                        <CalendarDays className="h-4 w-4 text-primary" /> Dia de Perna
                                                    </FormLabel>
                                                    <Select onValueChange={field.onChange} value={field.value}>
                                                        <FormControl><SelectTrigger className="bg-secondary/10"><SelectValue/></SelectTrigger></FormControl>
                                                        <SelectContent>
                                                    ...
                                                    <FormDescription className="text-[9px]">A IA evita intensidade nas 24h seguintes.</FormDescription>
                                                </FormItem>
                                            )} />
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            >
                                {context.planGenerationStatus === 'pending' ? <Loader2 className="animate-spin mr-2 size-4" /> : <Zap className="mr-2 size-4" />} 
                                Gerar Ciclo IA
                            </Button>
                        </div>
                    </form>
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl bg-card border border-border shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-[8px] text-muted-foreground uppercase font-black tracking-tighter">Performance</div>
                                </div>
                            </div>
                            <Switch 
                ...
                <Card className="bg-card/30">
                    <CardHeader>
                        <CardTitle className="text-lg flex items-center gap-2 font-headline uppercase italic tracking-tight">
                            <Target className="size-5 text-primary" /> Status
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                        ...
                        <p className="text-[9px] text-muted-foreground leading-tight italic">
                            O Gemini Coach utiliza estes dados para evitar o sobretreinamento.
                        </p>
                    </CardContent>
                </Card>
            </div>
